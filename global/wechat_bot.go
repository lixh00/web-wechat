...
func wechatMessageHandle(bot *protocol.Bot) {
	...
	textHandle := func(ctx *protocol.MessageContext) {
		if !ctx.IsSendBySelf() {
			sender, _ := ctx.Sender()
			context := protocol.FormatEmoji(ctx.Content)
			senderNickName := protocol.FormatEmoji(sender.NickName)
			if ctx.IsSendByGroup() {
				// 取出消息在群里面的发送者
				senderInGroup, _ := ctx.SenderInGroup()
				logger.Log.Infof("[群聊][收到新文字消息] == 发信人：%v[%v] ==> 内容：%v", senderNickName,
					protocol.FormatEmoji(senderInGroup.NickName), context)
			} else {
				logger.Log.Infof("[好友][收到新文字消息] == 发信人：%v ==> 内容：%v", senderNickName, context)
			}
		}
		ctx.Next()
	}
